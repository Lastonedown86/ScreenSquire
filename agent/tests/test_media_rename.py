...
def test_rename_happy_path(signed, media):
    r = signed("POST", "/api/media/old.jpg/rename", json={"new_name": "new"})
    assert r.json() == {"ok": True, "name": "new.jpg"}
    assert not (media / "old.jpg").exists()
    assert (media / "new.jpg").exists()


def test_rename_rewrites_playlist_and_bumps(agent_module, signed, media):
...
def test_rename_rewrites_dashboard_boards(agent_module, signed, media, monkeypatch):
...
def test_rename_duplicate_target_409(signed, media):
...
def test_rename_bad_names_400(signed, media):
    for bad in ["", "  ", "a/b", "a\\b", "..", "x" * 101]:
        r = signed("POST", "/api/media/old.jpg/rename", json={"new_name": bad})
        assert r.status_code == 400, bad


def test_rename_missing_file_404(signed, media):
    r = signed("POST", "/api/media/nope.jpg/rename", json={"new_name": "new"})
    assert r.status_code == 404


def test_rename_same_name_noop(signed, media):
    r = signed("POST", "/api/media/old.jpg/rename", json={"new_name": "old"})
