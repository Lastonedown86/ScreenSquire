...
def test_kiosk_start(agent_module, client, monkeypatch):
    seen = {}
    def script(cmd):
        seen["cmd"] = cmd
        return (0, "", "")
...
def test_kiosk_stop_maps_to_stop(agent_module, client, monkeypatch):
    seen = {}
...
def test_kiosk_failure_returns_error(agent_module, client, monkeypatch):
