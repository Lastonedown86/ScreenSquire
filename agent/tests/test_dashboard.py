...
def test_running_timer_gets_endsat_epoch_ms(client, signed):
    before = int(time.time() * 1000)
    r = signed("POST", "/api/dashboard", json={
        "view_data": {"boards": {}},
...
def test_boards_roundtrip(client, signed):
    signed("POST", "/api/dashboard", json={
...
def test_board_push_preserves_running_timer_endsat(client, signed):
    signed("POST", "/api/dashboard", json={"view_data": {"boards": {}},
    ...
    signed("POST", "/api/dashboard", json={"view_data": {"boards": {"pairings": "/media/p-1.png"}},
        "timer": {"state": "running", "remaining": 1500, "round": 1, "label": "Round 1"}})
    e2 = client.get("/api/dashboard").json()["timer"]["endsAt"]
    assert e2 == e1  # countdown NOT reset

def test_expired_timer_reanchors_on_repost(agent_module, client, signed):
...
def test_new_round_reanchors_timer(client, signed):
    signed("POST", "/api/dashboard", json={"view_data": {"boards": {}},
        "timer": {"state": "running", "remaining": 1500, "round": 1, "label": "Round 1"}})
    e1 = client.get("/api/dashboard").json()["timer"]["endsAt"]
    signed("POST", "/api/dashboard", json={"view_data": {"boards": {}},
...
def test_partial_board_push_merges(client, signed):
    signed("POST", "/api/dashboard", json={"view_data": {"boards": {"standings": "/media/s-1.png"}},
        "timer": {"state": "stopped"}})
    signed("POST", "/api/dashboard", json={"view_data": {"boards": {"pairings": "/media/p-9.png"}},
        "timer": {"state": "stopped"}})
