...
def test_detach_removes_playlist_items(agent_module, signed, media):
    ...
    r = signed("POST", "/api/media/old.jpg/detach").json()
...
def test_detach_clears_dashboard_boards(agent_module, signed, media, monkeypatch):
...
def test_detach_then_delete_succeeds(agent_module, signed, media):
    agent_module.state.playlist.items = [agent_module.PlaylistItem(type="image", source="old.jpg")]
    assert signed("DELETE", "/api/media/old.jpg").status_code == 409
    signed("POST", "/api/media/old.jpg/detach")
    assert signed("DELETE", "/api/media/old.jpg").status_code == 200
    assert not (media / "old.jpg").exists()


def test_detach_unused_file_noop(signed, media):
    r = signed("POST", "/api/media/old.jpg/detach").json()
